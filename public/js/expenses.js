...
async function loadMyExpenses() {
  try {
    ...
    const myExpenses = expenses.filter(e => e.employee_username === user.username);
    ...
    myExpenses.forEach(e => {
      ...
      tr.innerHTML = `
        ...
        <td>${e.description}</td>
        <td>${e.bill_filename ? `<a href="/uploads/${e.bill_filename}" target="_blank">View Bill</a>` : '-'}</td>
  ...
  try {
    ...
    pendingExpenses.forEach(e => {
      ...
      tr.innerHTML = `
        ...
        <td>${e.type}</td>
        <td>₹${parseFloat(e.amount).toFixed(2)}</td>
        <td>${e.description}<br>${e.bill_filename ? `<a href="/uploads/${e.bill_filename}" target="_blank">View Bill</a>` : 'No bill'}</td>
        <td>
